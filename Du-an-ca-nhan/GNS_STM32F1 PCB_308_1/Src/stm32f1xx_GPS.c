...
uint8_t full_78;
uint8_t OK,RRO,server_78,CONNECT,Rx_sta,UGNSINF[100],LIP[20],CPBR[100],ind_CPBR,GSN[16];//865067024139803//865067022455847//865067024142088
...
void chuyen_tiep_78(void);                              // chuyen tiep du lieu serve uart2
  ...
  switch (Rx_plus_sta)
  {
    case 1: // nhan du lieu tu server 7878...0d0a
    {
      if(ind_server<19) //Rx_server[20]
      {
        ...
        ind_server++;
        if(Rx_data[2]==0x0d&Rx_data[1]==0x0a&Rx_data[0]==0x0d) {full_78=1; Rx_plus_sta=0;} // nhan full data , ko nhan data
      }
...
void GPS_server(void) ///gui toa do GPS len server
{
  ...
  if(dem_0078>=6) //server ko tr loi lien tiep 6 lan
  {
    //HAL_UART_Transmit(&huart1,ket_thuc,1, 1000); 
    //HAL_Delay(500);
...
//chuyen tiep uart2
void chuyen_tiep_78(void)
{
  if(server_78==1 & full_78==1)
  {    
    HAL_UART_Transmit(&huart3,Rx_server,ind_server,1000);
    full_78=0;
  }
}
